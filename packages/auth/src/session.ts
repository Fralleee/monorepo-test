...
function deriveRoleFromGroups(groups: string[]): UserRole {
    if (groups.includes("internal_users")) {
        return "ADMIN";
    }
    return "USER";
}

export async function verifyToken(token: string, orgId?: string): Promise<SessionUser | null> {
...
export function extractTokenFromCookieHeader(cookieHeader: string | null | undefined, orgId?: string): string | null {
    if (!cookieHeader) {
        return null;
    }
