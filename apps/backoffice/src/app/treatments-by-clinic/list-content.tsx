								...
								{items.map((item) => (
									<TableRow key={item.id}>
										<TableCell className="font-medium">
											{item.clinic.name}
											<span className="ml-2 text-xs text-muted-foreground">({item.clinic.code})</span>
										</TableCell>
										<TableCell>{item.treatment.name}</TableCell>
										<TableCell className="text-muted-foreground">${Number(item.treatment.price).toFixed(2)}</TableCell>
										<TableCell>
											{item.priceOverride ? (
												<span className="font-medium text-green-600">${Number(item.priceOverride).toFixed(2)}</span>
											) : (
												'-'
											)}
										</TableCell>
										<TableCell className="max-w-[150px] truncate text-muted-foreground">{item.notes || '-'}</TableCell>
										<TableCell>
